...
app.post("/read", (req, res) => {
  const start = Number(req.body.start || 0); // startがundefinedの場合は0にする
...
app.post("/post", (req, res) => {
  ...
  const quote = req.body.quote || ""; // 引用を受け取る
  const timestamp = req.body.timestamp || new Date().toLocaleString(); // タイムスタンプを受け取るか現在時刻を使用

  console.log("新規投稿:", { name, message, quote, timestamp });

  // 本来はここでDBMSに保存する
  bbs.push({ name: name, message: message, quote: quote, timestamp: timestamp });
